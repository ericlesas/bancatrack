...
const chartResults = computed(() => [...props.results].sort((a, b) => b.month.localeCompare(a.month)))
const { visibleItems, hasMore, sentinel } = useProgressiveList(chartResults, 12)
...
<template>
  <section class="monthly-chart" aria-label="Gráfico de resultado mensal">
    ...
    <ul>
      <li v-for="item in visibleItems" :key="item.month">
    ...
    <p v-if="hasMore" ref="sentinel" class="scroll-hint">Mais meses serão carregados ao continuar</p>
  </section>
</template>
